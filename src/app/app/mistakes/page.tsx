...
export default async function MistakesPage() {
  ...
  if (clientResult.value) {
    ...
    mistakes = (data ?? []).map((row) => ({ id: row.id, concept: row.concept, subject: row.subjects?.name ?? "Unsorted", source: row.source ?? "Personal review", reason: row.mistake_reason, correction: row.corrected_reasoning, nextReviewLabel: row.next_review_on ? new Intl.DateTimeFormat("en-IN", { day: "numeric", month: "short" }).format(new Date(row.next_review_on)) : "When ready" }));
