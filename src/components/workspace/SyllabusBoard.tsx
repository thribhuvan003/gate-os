...
export function SyllabusBoard({ subjects, selectedSubjectId, onTopicChange, onSubjectChange }: SyllabusBoardProps) {
  ...
  return (
    <section className="workspace-page syllabus-board" aria-labelledby="syllabus-title">
      <header className="workspace-page-header"><div><p className="workspace-eyebrow"><BookOpenCheck aria-hidden="true" /> GATE CS/IT 2027 target</p><h1 id="syllabus-title">Know the syllabus. Own the sequence.</h1><p>This versioned GATE 2027 working catalog keeps your progress intact. When the examination authority publishes a final syllabus, GATE OS will add a reviewed version instead of silently changing completed topics.</p></div>{selectedSubject ? <span className="workspace-status-label">{totals.complete} / {totals.total} complete</span> : null}</header>
