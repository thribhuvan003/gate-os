...
export default async function VaultPage() {
  ...
  if (clientResult.value) {
    ...
    documents = (data ?? []).map((item: { id: string; title: string; tags: string[]; is_pinned: boolean; last_read_page: number | null; updated_at: string; subjects: { name: string } | null }) => ({ id: item.id, title: item.title, subject: item.subjects?.name ?? "Unsorted", tags: item.tags, pinned: item.is_pinned, page: item.last_read_page ?? undefined, updatedLabel: new Intl.DateTimeFormat("en-IN", { day: "numeric", month: "short" }).format(new Date(item.updated_at)) }));
