...
export function SyllabusBoard({ subjects, selectedSubjectId, onTopicChange, onSubjectChange }: SyllabusBoardProps) {
  ...
  return (
    <section className="workspace-page syllabus-board" aria-labelledby="syllabus-title">
      <header className="workspace-page-header"><div><p className="workspace-eyebrow"><BookOpenCheck aria-hidden="true" /> GATE CS/IT 2027 target</p><h1 id="syllabus-title">Know the syllabus. Own the sequence.</h1><p>Complete official-source 2026 subject, section, topic and subtopic wording is the reviewed working baseline. The final 2027 release will create a new version rather than silently changing progress.</p></div>{selectedSubject ? <span className="workspace-status-label">{totals.complete} / {totals.total} complete</span> : null}</header>
