...
export default async function CirclesPage() {
  ...
  if (clientResult.value) {
    ...
    circles = (data ?? []).map((row) => {
      ...
      return { id: row.id, name: row.name, goal: row.description ?? "Focus together, keep personal work private.", members: (row.circle_memberships ?? []).map((member) => ({ id: member.user_id, name: member.profiles?.display_name ?? "Member", status: "offline" as const })), activeSession: active ? { title: active.goal, endsAt: active.ends_at ? new Intl.DateTimeFormat("en-IN", { hour: "numeric", minute: "2-digit" }).format(new Date(active.ends_at)) : "soon", reactions: 0 } : undefined };
    });
  }
