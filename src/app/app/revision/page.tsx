...
export default async function RevisionPage() {
  ...
  if (clientResult.value) {
    const { data } = await clientResult.value.from("revision_items").select("id,title,source_type,due_on,completed_at,subjects(title)").order("due_on");
    items = (data ?? []).map((row) => ({ id: row.id, title: row.title, subject: row.subjects?.[0]?.title ?? "General", dueLabel: new Intl.DateTimeFormat("en-IN", { weekday: "short", day: "numeric", month: "short" }).format(new Date(row.due_on)), completed: Boolean(row.completed_at), source: row.source_type === "manual" || row.source_type === "reflection" ? "topic" : row.source_type as "topic" | "note" | "mistake" }));
  }
